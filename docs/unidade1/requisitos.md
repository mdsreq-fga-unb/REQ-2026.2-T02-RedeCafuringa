...
!!! info "Análise e Consenso"
    ...
    - **Participantes:** Engenheiros de Requisitos, Cliente, Amostra de Produtores Rurais e Consumidores.
...
| :--- | :-------------- | :---------------- | :------------------ | :----------------- |
| **Concepção** | Elicitação e Descoberta | Capturar dores e necessidades do ecossistema local. | Entrevistas Semiestruturadas | Transcrições de áudio/texto e lista bruta de necessidades de negócio. |
...
| | Declaração de Requisitos | Sintetizar necessidades em OEs, CPs e Épicos Iniciais. | Redação Estruturada & Mapeamento de Épicos | Minuta do Documento de Visão e Lista Inicial de Épicos. |
...
| | Verificação e Validação | Apresentar OEs e CPs para confirmar premissas com stakeholders. | Walkthrough com Stakeholders | Termo de Aceite do Escopo Inicial e Registro de Decisões. |
| | Organização e Atualização | Indexar OEs, CPs e Épicos e atribuir estado de aprovação. | Atribuição de Metadados & Versionamento | Baseline 0 - Escopo de Visão Inicial. |
...
| | Declaração de Requisitos | Estruturar Épicos em Histórias de Usuário com critérios de aceite. | Decomposição Funcional de Épicos em User Stories | Backlog de User Stories preliminares. |
...
| | Organização e Atualização | Vincular Necessidades, Épicos, User Stories, RNFs e protótipo. | Mapeamento de Rastreabilidade Bidirecional | Matriz de Rastreabilidade - Versão Inicial. |
...
| | Declaração de Requisitos | Redigir cenários de comportamento e dados de entrada nos cartões do Kanban. | Detalhamento Just-In-Time (JIT) de Critérios de Aceite | Cartões de User Stories preenchidos com critérios de aceite completos (Atendimento ao DoR - Definition of Ready). |
| | Análise e Consenso | Negociar ajustes e trocas no backlog com base no aprendizado dos incrementos. | Repriorização Adaptativa de Fluxo | Backlog do Kanban repriorizado. |
